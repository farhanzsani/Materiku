// server/api/materi/[id].put.ts
import { PrismaClient } from '@prisma/client'
const prisma = new PrismaClient()

export default defineEventHandler(async (event) => {
    // 1. CEK LOGIN
    const authorId = getCookie(event, 'auth_token')
    if (!authorId) {
        throw createError({
            statusCode: 401,
            statusMessage: 'Harus login admin untuk mengedit materi!'
        })
    }

    const id = getRouterParam(event, 'id')

    if (!id) {
        throw createError({
            statusCode: 400,
            statusMessage: 'ID materi tidak ditemukan'
        })
    }

    const body = await readBody(event)

    // Generate slug baru dari title
    const slug = body.title
        .toLowerCase()
        .replace(/[^\w ]+/g, '')
        .replace(/ +/g, '-')

    try {
        // Cek apakah materi ada
        const existing = await prisma.materi.findUnique({
            where: { id }
        })

        if (!existing) {
            throw createError({
                statusCode: 404,
                statusMessage: 'Materi tidak ditemukan'
            })
        }

        // Update materi
        const materi = await prisma.materi.update({
            where: { id },
            data: {
                title: body.title,
                slug: slug,
                content: body.content
            }
        })

        return { message: 'Materi berhasil diupdate!', data: materi }
    } catch (e: any) {
        if (e.statusCode) throw e
        throw createError({
            statusCode: 500,
            statusMessage: 'Gagal mengupdate materi di database'
        })
    }
})
